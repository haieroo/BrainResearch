...
matdataPath = [fpath fname(1:end-4) 'data.mat'];
...
for ii=1:ExtHeaderCount
    ...
    switch PacketID
        ...
        case 'NSASEXEV' %% Not implemented in the Cerebus firmware. 
            ...
            NSAS.AnalCh5Detect = typecast(ExtendedHeader(25:26),'uint16');
        otherwise
...
NEV.Data.Spikes.Timestampsy = double(NEV.Data.Spikes.Timestamps)/30000; % time(s)
...
%% sort by sunyang
j=1;
for i=min(NEV.Data.Spikes.Electrode):max(NEV.Data.Spikes.Electrode)
    clear datat
    datat=NEV.Data.Spikes.Timestampsy(NEV.Data.Spikes.Electrode==i);
    if size(datat,1)>0
    data{1,j}=datat;
    data{2,j}=strcat('electode_',num2str(i));
    j=j+1;
    end    
end
clear i j datat
